...
import com.example.iottest10_28_19_11.adapter.FollowingDeviceAdapter;
...
public class DevicesFragment extends Fragment {
    ...
    private FollowingDeviceAdapter followingDeviceAdapter;
    ...
    private void fetchDevices() {
        ...
        apiService.getAllDevices(token).enqueue(new Callback<DeviceResponse>() {
            @Override
            public void onResponse(@NonNull Call<DeviceResponse> call, @NonNull Response<DeviceResponse> response) {
                if (response.isSuccessful() && response.body() != null) {
                    List<Device> devices = response.body().getDevices();
                    if (devices != null && !devices.isEmpty()) {
                        followingDeviceAdapter = new FollowingDeviceAdapter(getContext(),devices);
                        recyclerViewDevices.setAdapter(followingDeviceAdapter);
                    } else {
